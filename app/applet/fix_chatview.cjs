...
const replacement = `export function ChatView() {
  ...
  useEffect(() => {
      const darlingBot = {
          name: 'Darling',
          images: ['https://ui-avatars.com/api/?name=Darling&background=ff2d55&color=fff&size=150']
      };
      
      const matchedChats = [darlingBot, ...likedProfiles];
      ...
      setChats(matchedChats.map((c: any, i: number) => {
         ...
         if (c.name === 'Darling' && lastMsg === 'Bắt đầu trò chuyện...') {
             lastMsg = 'Xin chào! Tôi có thể giúp gì cho tình yêu của bạn?';
             isUnread = true;
         }
          ...
          >
             <div className="bg-[#ff9ca0] h-[90px] pt-[40px] px-4 flex items-center gap-3 shrink-0 shadow-sm z-10 sticky top-0 border-b border-black/10">
                ...
                <div className="flex-1 flex flex-col cursor-pointer" onClick={() => setShowInfo(true)}>
                   <span className="font-bold text-[17px] leading-tight text-black">
                       {activeChat.name}
                       {activeChat.name === 'Darling' && <Zap className="w-4 h-4 ml-1 inline text-[#ff2d55] fill-[#ff2d55]" />}
                   </span>
                   <span className="text-[12px] font-medium text-black/70">{activeChat.name === 'Darling' ? 'Đang online' : 'Đang hoạt động'}</span>
                </div>
             ...
             <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-[#fff0f1] pb-32">
                ...
                {activeChat.name === 'Darling' && messages.length === 0 && (
                   <div className="text-center mb-6">
                      <div className="mx-auto w-16 h-16 bg-[#ff2d55]/10 rounded-full flex items-center justify-center mb-2">
                         <Zap className="w-8 h-8 text-[#ff2d55] fill-[#ff2d55]"/>
                      </div>
                      <p className="text-[14px] text-gray-600 font-medium">Tôi là Darling, chuyên gia tư vấn tình cảm của bạn.</p>
                      <p className="text-[13px] text-gray-400 mt-1">Hãy hỏi tôi bất kỳ điều gì về tình yêu và hẹn hò nhé!</p>
                   </div>
                )}
                       ...
                       <div className="flex flex-col gap-1 max-w-[80%]">
                          <div style={{ backgroundColor: isMe && !msg.isAudio && !msg.isImage ? chatTheme : (msg.isAudio || msg.isImage ? 'transparent' : '#ffffff') }} className={\`px-4 py-2.5 shadow-sm text-[15px] \${isMe ? 'text-white rounded-l-2xl rounded-tr-2xl rounded-br-sm font-medium' : 'bg-white border border-black/5 text-black rounded-r-2xl rounded-tl-2xl rounded-bl-sm'} \${msg.isImage || msg.isAudio ? 'p-0 shadow-none border-none' : ''}\`}>
                            {msg.isImage ? (
                              ...
                              <div className="markdown-body prose prose-sm" dangerouslySetInnerHTML={{ __html: msg.text.replace(/\\n/g, '<br/>') }} />
                           )}
                          </div>
                          {msg.sender === 'them' && activeChat.name === 'Darling' && (
                             <div className="text-[10px] text-gray-400 mt-0.5 ml-1 flex items-center gap-1">
                                <Zap className="w-3 h-3 text-[#ff2d55]" /> Darling AI
                             </div>
                          )}
                       </div>
                     </div>
                ...
                {isTyping && (
                     <div className="flex items-end gap-2 mt-4">
                       <img src={activeChat.img} className="w-[28px] h-[28px] rounded-full object-cover mb-1 shadow-sm border border-black/5 shrink-0" />
                       <div className="flex gap-1.5 bg-white border border-black/5 px-4 py-3 rounded-r-2xl rounded-tl-2xl rounded-bl-sm shadow-sm opacity-80">
              ...
              <div className="flex-1 overflow-y-auto pb-20">
                 <div className="flex flex-col items-center pt-8 pb-6 bg-white border-b border-black/5">
                    ...
                    <span className="text-[15px] font-medium text-black/50 mt-1">
                        {activeChat.name === 'Darling' ? 'Khách hàng VIP' : 'Darling User'}
                    </span>
                       ...
                       {showMedia && (
                         <div className="px-4 pb-4">
                            {chatImages.length > 0 ? (
                                ...
                                <div className="text-[14px] text-gray-400 text-center py-4">Chưa có hình ảnh nào được gửi</div>
                            )}
                         </div>
                       )}
                    </div>
               ...
               <div className="flex-1 pt-1 overflow-hidden pr-2">
                 <div className="flex justify-between items-center mb-1">
                   <h3 className={\`font-bold text-[16px] \${chat.isUnread ? 'text-[#ff2d55]' : 'text-black'}\`}>
                       {chat.name}
                       {chat.name === 'Darling' && <span className="ml-2 text-[10px] bg-[#ff2d55] text-white px-1.5 py-0.5 rounded-sm font-bold uppercase">AI</span>}
                   </h3>
                 </div>
